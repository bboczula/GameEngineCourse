...
void Sapphire::OutputFile::Write(LPCVOID data, DWORD dataSizeInBytes)
{
	...
	BOOL errorFlag = WriteFile(fileHandle, data, dataSizeInBytes, &bytesWritten, nullptr);

	if (FALSE == errorFlag)
	{
		MessageBoxA(nullptr, "Unable to write to file!", "File Error", MB_OK | MB_ICONERROR);
		exit(1);
	}
	else if (bytesWritten != dataSizeInBytes)
	{
		MessageBoxA(nullptr, "Unable to write entire string!", "File Error", MB_OK | MB_ICONERROR);
		exit(1);
	}
}
