...
GpuApi::DX12Resource::DX12Resource(void* source, D3D12_RESOURCE_STATES state)
	: resource(static_cast<ID3D12Resource*>(source)), state(state)
{
}

GpuApi::DX12Resource::DX12Resource(DX12Device* device, UINT64 bufferSize)
...
GpuApi::DX12Resource::DX12Resource(DX12Device* device, DXGI_FORMAT textureFormat, UINT width, UINT height)
...
GpuApi::DX12Resource::DX12Resource(DX12Device* device, DXGI_FORMAT textureFormat, UINT width, UINT height, D3D12_RESOURCE_FLAGS flags)
	: state(D3D12_RESOURCE_STATE_COPY_DEST)
{
	auto heapProps = CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_DEFAULT);
	auto desc = CD3DX12_RESOURCE_DESC::Tex2D(textureFormat, width, height);
...
GpuApi::DX12Resource::~DX12Resource()
{
	SafeRelease(&resource);
}

void GpuApi::DX12Resource::Upload(void* source, size_t size)
{
	UINT8* resourceData;
...
void GpuApi::DX12Resource::Upload(float* source, UINT width, UINT heigh, UINT pixelSize, UINT alignedRowPitch)
{
	UINT8* resourceData;
	CD3DX12_RANGE readRange(0, 0);
	ExitIfFailed(resource->Map(0, &readRange, reinterpret_cast<void**>(&resourceData)));
...
D3D12_GPU_VIRTUAL_ADDRESS GpuApi::DX12Resource::GetGpuVirtualAddress()
{
	return resource->GetGPUVirtualAddress();
}

ID3D12Resource* GpuApi::DX12Resource::GetResource()
{
	return resource;
}
